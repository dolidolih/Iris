...
import party.qwer.iris.model.SetConfigRequest
...
class HttpServerKt(
    ...
    fun startServer() {
        embeddedServer(Netty, port = 8080) {
            ...
            routing {
                get("/config") {
                    val html = ConfigPageDocumentProvider.renderPage(Configurable.getInstance())
                ...
                get("/config/info") {
                    val config = Configurable.getInstance()
                    val configJson = JSONObject()

                    try {
                        configJson.put("bot_name", config.botName)
                        configJson.put("bot_http_port", config.botSocketPort)
                        configJson.put("web_server_endpoint", config.webServerEndpoint)
                        configJson.put("db_polling_rate", config.dbPollingRate)
                        configJson.put("message_send_rate", config.messageSendRate)
                        configJson.put("bot_id", config.botId)
                    } catch (e: JSONException) {
                        throw Exception("Failed to serialize DB status to JSON: $e")
                    }
                ...
                post("/config/{name}") {
                    ...
                    val req = call.receive<SetConfigRequest>()
                    val configurable = Configurable.getInstance()
                    ...
                    when (name) {
                        ...
                        "dbrate" -> {
                            val value = req.rate?.toLongOrNull()
                        ...
                        "botport" -> {
                            val value = req.rate?.toIntOrNull()
                                ?: throw Exception("missing or invalid value")
                ...
                post("/reply") {
                    ...
                    when (replyRequest.type) {
                        ReplyType.TEXT -> Replier.SendMessage(
                            notificationReferer, roomId, replyRequest.data.content
                        )

                        ReplyType.IMAGE -> Replier.SendPhoto(roomId, replyRequest.data.content)
                ...
                post("/decrypt") {
                    ...
                    val plaintext = KakaoDecrypt.decrypt(
                        ...
                        decryptRequest.user_id ?: Configurable.getInstance().botId
                    )
    ...
    private fun processDecryptionForResponse(rowJson: JSONObject) {
        try {
            if (rowJson.has("message") || rowJson.has("attachment")) {
                ...
                if (!vStr.isEmpty()) {
                    try {
                        ...
                        val userId = rowJson.optLong("user_id", Configurable.getInstance().botId)
            ...
            val botId = Configurable.getInstance().botId
